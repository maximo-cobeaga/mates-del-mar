...
def make_paypal_payment(amount, currency, return_url, cancel_url):
    ...
    token_payload = {'grand_type': 'client_credentials'}
    token_header = {'Accept': 'application/json', 'Accept-Lenguage': 'en_US'}
    token_response = requests.post(token_url, auth=(
        client_id, secret), data=token_payload, headers=token_header)
    if token_response.status_code != 200:
        return False, "Fallo la autenticacion de PAYPAL API", None
    ...
    if payment_response.status_code != 201:
        return False, "Hubo un fallo al crear paypal pago", None
...
def verify_paypal_payment(payment_id):
    ...
    if token_response.status_code != 200:
        raise Exception('2 Failed to authenticate with PayPal API.')
    ...
    if payment_status == 'approved':
        ...
        payer_email = payment_details_response.json()[
